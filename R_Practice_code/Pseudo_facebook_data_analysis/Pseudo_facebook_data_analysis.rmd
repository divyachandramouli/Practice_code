...
### Generate box plots of friend count by gender and see the difference between medians
y axis is friend count and x axis is categorical variable - gender
```{r Box Plots}
qplot( x = gender, y = friend_count, data = subset(pf,!is.na(gender)), geom = 'boxplot')
```
Boxes cover the inter quartile range - so many outliers
...
```
Female box is just below 250 - this may or may not be accurate because we removed data points

Let's use the cored cartesian layer and do the same
```{r}
